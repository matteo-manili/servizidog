...
<head>
    <title><fmt:message key="adozione.title"/></title>
...
<script>
	var visualizzaCivico = '${adozione.visualizzaCivico}';
	var cittaSalvata = '${adozione.address.city}';
</script>




<spring:bind path="adozione.*">
...
<section>
	<div class="row">
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center padding">
			<div class="content">
				<h1><fmt:message key="adozione.adozionecane.title"/></h1>
				<hr>
			</div>
		</div>
	</div>
	...
	<div class="row">
		<div class="col-md-6 col-md-offset-3 text-center padding">
			<ul class="nav nav-tabs responsive" id="tabs">
			  <li role="presentation" class="active"><a data-toggle="tab" href="#info"><fmt:message key="adozione.infogenerali"/></a></li>
			  <li role="presentation"><a data-toggle="tab" href="#inserisciCane"><fmt:message key="adozione.inseriscicane"/></a></li>
			  <li role="presentation"><a data-toggle="tab" href="#foto"><fmt:message key="adozione.foto"/></a></li>
			</ul>
			<div class="tab-content responsive">


				<div role="tabpanel" class="tab-pane active" id="info">
					<form:form commandName="adozione" method="post" action="adozioneform" id="adozioneForm" autocomplete="off" onsubmit="return validateAdozione(this)">
					
					<form:hidden path="id"/>
						<spring:bind path="adozione.descrizione">
					    	<div class="col-md-12 col-sm-12 col-xs-12 text-left margin-top${(not empty status.errorMessage) ? ' has-error' : ''}">
					    </spring:bind>
					        <label><fmt:message key="adozione.descrizione"/></label>
			           	...
			           	<div class="col-md-12 col-sm-12 col-xs-12 text-left margin-top">
			           		<label><fmt:message key="adozione.telefono"/></label>
						    <form:input  path="telefono" id="telefono" /> 
				      	</div>
				      	

				     	<spring:bind path="adozione.indirizzoCompleto">
					    	<div class="col-md-12 col-sm-12 col-xs-12 text-left margin-top${(not empty status.errorMessage) ? ' has-error' : ''}">
					    </spring:bind>
					    	<label><fmt:message key="adozione.indirizzo"/></label>
					    	<form:input  path="indirizzoCompleto" id="indirizzoCompleto"  />
					    	<form:errors path="indirizzoCompleto" cssClass="help-block"/>
						    <!-- questa è la mappa -->
						    <label><fmt:message key="adozione.mappa"/></label>
						...
						<div class="col-md-12 col-sm-12 col-xs-12 text-right margin-top">
							<a class="btn btn-green" name="save" onclick="validazioneSubmit( adozioneForm );">
						...
						(function( $ ) {
							...
							if ('${adozione.visualizzaCivico}' == 'true' ){
							...
							$('#us5').locationpicker({
								location: {latitude: '${adozione.address.lat}', longitude: '${adozione.address.lng}'},
								zoom: 14,
						        radius: 400,
			    ...
			    <div role="tabpanel" class="tab-pane" id="inserisciCane">
			        ...
			        <%@ include file="/common/adozioneCane.jsp" %>
...
<c:set var="scripts" scope="request">
	...
	function onFormSubmit(theForm) {
		alert("validateAdozione");
	    return validateAdozione(theForm);
	}
	</script>
</c:set>

<v:javascript formName="adozione" staticJavascript="false"/>
