...
public class UpdatePasswordController extends BaseFormController {
    ...
    @RequestMapping(value = "/requestRecoveryTokenUsernameAndPassword*", method = RequestMethod.POST)
    public String requestRecoveryTokenUsernameAndPassword(
            @RequestParam(value = "email", required = true) final String email,
            final HttpServletRequest request) {
        log.debug("Sending recovery token to user " + email);
        try {
            getUserManager().sendUsenameAndPasswrdRecoveryEmail(email, RequestUtil.getAppURL(request) + RECOVERY_PASSWORD_TEMPLATE);
        } catch (final UsernameNotFoundException ignored) {
            // lets ignore this
        }
        saveMessage(request, getText("recoverPass.RecoveryTokenUsernameAndPassword.sent", request.getLocale()));
        return "redirect:/login";
    }
